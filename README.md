...
주소를 열면 상단 바에 **📲 버튼**이 뜬다. 누르면 설치된다. (설치 후에는 버튼이 자동으로 사라진다.)

| 기기 | 방법 |
|---|---|
| **안드로이드 (크롬)** | 📲 버튼 → "설치" |
| **윈도우/맥 (크롬·엣지)** | 📲 버튼, 또는 주소창 오른쪽 설치 아이콘 |
| **아이폰/아이패드** | ⚠️ **사파리로 열어야 함** → 공유 버튼(↑) → "홈 화면에 추가" |

iOS는 브라우저가 설치 프롬프트를 지원하지 않아서 📲 버튼을 누르면 안내 화면이 뜬다. 크롬으로 열면 설치가 안 되니 사파리를 써야 한다.

설치하면 주소창 없이 앱처럼 뜨고, 홈 화면·시작 메뉴에서 바로 열리며, 오프라인에서도 실행된다.
아이콘을 길게 누르면 **준비물 · 예산 · 지도**로 바로 가는 바로가기가 나온다.
